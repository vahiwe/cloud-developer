...
describe('concat', () => {
  ...
  it('should concatenate the strings', () => {
    const result = concat("Hello ","Sam");
    expect(result).to.equal("Hello Sam");
  });

  it('should concatenate the strings', () => {
    const result = concat("Hello ","John");
    expect(result).to.equal("Hello John");
  });

  it('should throw an error if empty string is passed as parameter', () => {
    expect(()=>{ concat("","John") }).to.throw('Empty String')
  });
